...
const post: NextApiHandler = async (req, res) => {
  ...
  const user = await app.container.userUseCase.create(new LoginId(loginId), Password.from(password));
  const session = await app.container.userUseCase.login(user.userId, user.password);
